...
router.post("/click", async function (req, res, next) {
  ...
  try {
    ...
    const timestampsSnapshot = await database.get(
      database.child(database.ref(db), `users/${req.body.userid}/timestamps`)
    );
    let timestampList = [];
    ...
    let updates = {};
    updates[
      `/users/${req.body.userid}/timestamps/${currentTimeInEpochSeconds}`
    ] = true;
    ...
    // Compute Bonus coins
    const bonusSnapshot = await database.get(
      database.child(database.ref(db), `bonus`)
    );
    ...
    const coinsSnapshot = await database.get(
      database.child(database.ref(db), `users/${req.body.userid}/total_coins`)
    );
    if (newCoins > 0) {
      let totalCoins = 0;
      if (coinsSnapshot.exists())
        totalCoins = parseInt(coinsSnapshot.val()) + newCoins;
